<?php
return [
    'Home' => 'الصفحة الرئيسية',
    'Signup' => 'تسجيل',
    'Login' => 'الدخول',
    'Governorate' => 'المحافظة',
    'Nationality' => 'الجنسية',
    'Logout' => 'تسجيل خروج',
    'logout' => 'تسجيل خروج',
    'Name_Ar' => 'الاسم بالعربي',
    'Name' => 'الاسم',
    'Phone' => 'هاتف',
    'Area' => 'المنطقة',
    'Note' => 'ملاحظة',
    'Save' => 'حفظ',
    'Search' => 'بحث',
    'Reset' => 'ريست',
    'Create_Area' => 'انشاء مطقة',
    'Areas' => 'المناطق',
    "Description" => "الوصف",
    'Governorates' => 'المحافظات',
    'Create_Governorate' => 'انشاء محافظة',
    'Nationalities' => 'الجنسيات',
    'Create_Nationality' => 'انشاء جنسية',
    'Update' => 'تحديث',
    'Delete' => 'حذف',
    'Email' => 'البريد الاليكتروني',
    'Username' => 'اسم المستخدم',
    "RememberMe" => "تذكرني",
    'Password' => 'كلمة السر',
    'Remember_Me' => 'تذكرني',
    'Conf_Password' => 'تاكيد كلمة السر',
    'Pass_Dont_match' => 'تاكيد كلمة السر ليست ككلمة السر',
    'Back' => 'الرجوع',
    'Update_Nationality' => 'تحديث الجنسية',
    'Title' => 'العنوان',
    'Body' => 'الوصف',
    'Send_Save' => 'ارسال وحفظ',
    'Category' => 'الصنف',
    'All' => 'الكل',
    'Enter_date' => 'ادخل التاريخ',
    'Avatar' => 'صوره شخصية',
    'Must_Contain_10_Digits' => 'يجب ان يكون 10 ارقام',
    'Created_At' => 'تاريخ الانشاء',
    'User_ID' => 'اسم المستخدم',
    "About" => "عنا",
    "Social" => "مواقع تواصل اجتماعي",
    "Updated_At" => "تاريخ التحديث",
    "Name_full" => "الاسم من اربع مقاطع",
    "Courses" => "الدورات",
    "From" => "من",
    "To" => "الى",
    "Plz_Select_Year" => "ارجو تحديد السنة",
    "Plz_Select_Month" => "ارجو تحديد الشهر",
    "Add" => "اضافة",
    "IF_Exist" => "ان وجد",
    "My_Request" => "طلبي",
    "Successfuly_Message_Update" => "تم تعديل بنجاح ",
    "Successfully_Change_Password" => "تم تغير كلمة السر بنجاح",
    "And" => "و",
    "Status" => "الحالة",
    "Update_Area" => "تعديل منطقة",
    "Update_Category" => "تعديل الصنف",
    "Create_Degrees" => "انشاء شهادة",
    "Must_Be_Login" => "يجب عليك تسجيل الدخول",
    "Country" => "البلد",
    "Region" => "المحافظة",
    "Plz_Select" => "ارجو التحديد",
    "Yor_Applayed" => "لقد تقدمت لهذه الوظيفة",
    "Successfuly_Applay" => "تم التقديم بنجاح",
    "Products" => "المنتجات",
    "Create_Product" => "انشاء منتج",
    "Thumbnail" => "الصورة الرئيسية",
    "Purchasing_Price" => "سعر الشراء",
    "Selling_Price" => "سعر البيع",
    "Region_ID" => "المقاطعة أو المحافظة",
    "Name_En" => "الاسم بالانجليزية",
    "Update" => "تحديث",
    "Regions" => "المقاطعات او المحافظات",
    "Countries" => "البلدان",
    "Create_Country" => "انشاء بلد",
    "Country_Code" => "رمز الدولة",
    "Nationality_En" => "الجنسية بالانجليزي",
    "Nationality_Ar" => "الجنسية بالعربي",
    "Price_Delivery" => "سعر توصيل",
    "Create_Region" => "انشاء مقاطعة او محافظة",
    "Create_Category" => "انشاءالصنف",
    "Categorises" => "الاصناف",
    "Units" => "الوحدة",
    "Create_Unit" => "انشاء وحدة",
    "Statuses" => "الحالات",
    "Create_Status" => "انشاء حالة",
    "Warehouse" => "المستودع",
    "Localtion" => "العنوان",
    "Create_Warehouse" => "انشاء مستودع",
    "Warehouses" => "المستودعات",
    "Suppliers" => "المزودين",
    "Create_Supplier" => "انشاء مزود",
    "Site" => "الموقع الاليكتروني",
    "Location" => "العنوان",
    "Other_Phone" => "هاتف اخر",
    "Create_User" => "انشاء زبون",
    "Address" => "العنوان",
    "Additional" => "الاضافات",
    "Users" => "المستخدمين",
    "Products" => "المنتجات",
    "Create_Product" => "انشاء منتج",
    "thumbnail" => "صورة",
    "Purchasing_Price" => "سعر الشراء",
    "Selling_Price" => "سعر البيع",
    "Quantity" => "العدد",
    "Supplier" => "المزود",
    "Unit" => "الوحدة",
    "Images_Product" => "صور اضافية للمنتج",
    "Type" => "نوع",
    "Count" => "العدد",
    "Orders" => "الطلبات",
    "Order_Id" => "رقم الطلب",
    "Delivery_Date" => "تاريخ التسليم",
    "Delivery_Time" => "وقت التسليم",
    "Discount" => "مبلغ الخصم",
    "Delivery_Price" => "مبلغ التوصيل",
    "Total_Price" => "المبلغ كامل",
    "Amount_Required" => "المبلغ المطلوب",
    "Create_Order" => "انشاء طلب",
    "Orders" => "<<  <<<  الطلب >>>  >>>",
    "Product" => "المنتج",
    "Sub_Product" => "المنتج الفرعي",
    "Add_Sub_Product" => "اضافة منتج فرعي",
    "Sub_Product_Id" => "المنتج الفرعي",
    "Add_Product" => "اضافة منتج",
    "Quantity_All" => "العدد الكلي",
    "Quantity_Item" => "عدد الكلي للقطعة",
    "video_url" => "رابط الفيديو",
    "Price" => "السعر",
    "Video_Url" => "رابط يوتيوب",
    "Profit_Margin" => "هامش الربح",
    "Name_In_Facebook" => "الاسم على الفيسبوك",
    "Note" => "ملاحظة",
    'SubProductCount' => "افرع المنتج",
    'Create_SubProductCount' => "انشاء فرع منتج",
    "Number" => "عدد",
    "Text" => "نص",
    "Type_Options" => "خيارت المنتج",
    "Options_Sell_Products" => "خيارات بيع المنتجات",
    "Create_Options_Sell_Products" => " اضافة خيارات بيع المنتجات",
    "More_Info" => "تفاصيل أكثر",
    "Order_Now" => "اطلبها الان",
    "Choose_Offer" => "اختر العرض المناسب لك",
    "Choose_Type" => "اختار النوع",
    "Successful_Purchase" => "تم الشراء بنجاح",
    "Product_Suggested" => "منتجات مقترحة",
    "Error" => "حدث خطأ",
    "Success_For_Name" => "تم تسجيل طلبك بنجاح, شكراً لك",
    "Order_Price_Total" => "قيمة الطلب شامل التوصيل",
    "Order_Conected" => "سيتم التواصل معك خلال 48 ساعة لتأكيد طلبك",
    "See_Other_Product" => "شاهد عروض ومنتجات أخرى بأسعار خاصة لتصلك المنتجات بشحنة واحدة!!",
    'Check_Phone' => 'تاكد من ان الهاتف مكتوب بشكل صحيح',
    "Dashboard" => "لوحة التحكم",
    'Delivery_Price_Added' => "يضاف سعر التوصيل",
    "Need_Product" => "اريد",
    'Delivery_Fees' => "مصاريف التوصيل",
    'Total_Amount' => 'المبلغ الإجمالي',
    "Invoice" => "فاتورة",
    'Name_Product' => "اسم المنتج",
    'ID' => "رقم ",
    "SubTotal" => "المجموع الفرعي",
    "Print_All_Invoice" => "طباعة لالفواتير",
    "Export_PDF" => "استخراج ملف pdf",
    "Import" => "ادراج",
    'Company_Delivery' => 'شركات التوصيل',
    'Price_Company_Delivery' => 'سعر توصيل الشركة',
    'adminEmail' => 'adminEmail',
    'senderEmail' => 'senderEmail',
    'senderName' => 'senderName',
    'bsDependencyEnabled' => 'bsDependencyEnabled',
    'facebook_id' => 'facebook id',
    'sanpchat_id' => 'sanpchat id',
    'sanpchat_email' => 'sanpchat email',
    'tiktok_id' => 'tiktok id',
    'Settings' => 'الاعدادات',
    'Create_Company_Delivery' => 'انشاء شركة توصيل',
    'Company_Deliveries' => 'شركات النوصيل',
    'Logo' => 'Logo',
    'Create_Company_Delivery' => 'انشاء شركة توصيل',
    "Change_Password" => "تغير كلمة السر",
    "Old_Password" => "كلمة سر قديمة",
    "New_Password" => "كلمة السر الجديدة",
    "Confirm_Pass" => "اكد المرور",
    "Succ_Mess_Pass" => "تم تغير كلمة السر بنجاح",
    "Name_Of_Store" => "اسم المتجر",
    "Massage_Whatsapp" => "رسالة الوتس اب",
    "Successfuly_Update_Order" => "تم تعديل الطلب بنجاح",
    "Successfuly_Create_Order" => "تم اضافة الطلب بنجاح",
    "The_Sales" => "المبيعات",
    "The_Outlay" => "المصاريف",
    "Best_Seller" => "الافضل مبيع",
    "The_Orders" => "الطلبات",
    "Outlays" => "النفقات",
    "Create_Outlay" => "انشاء نفقة",
    "Create_Type_Outlay" => "انشاء نوع مصروف",
    "Type_Outlay" => "نوع المصروق",
    "Value" => "القيمة",
    "Date" => "تاريخ",
    "Count_Orders" => "عدد الطلبات",
    "Month" => "الشهر",
    "History_Status" => "تاريخ الحالة الطلبات",
    "Number_Of_Grains" => "عدد الحبات",
    'Time' => "الوقت",
    "Previous_Order_Statuses" => "حالات الطلبات السابقة",
    "Export_To_Driver" => "تصدير الى السائق",
    "Campany_Delivery" => "شركات التوصيل",
    "Main" => "الرئيسي",
    "Total_Sales" => "المبيعات",
    "Reviews" => "التقيم",
    "Color_Site" => 'لون الموقع',
    "Font_Color" => "لون الخط",
    "Background_Duration" => "دوران الرأس",
    "Background_First_Color" => "اللون الاول",
    "Background_Second_Color" => "اللون الثاني",
    "Theme_Settings" => "العدادات القالب",
    "Orderss" => "الطلبات",
    "Profile" => "الملف الشخصي",
    "Statistics" => "الاحصائيات",
    "Customers" => "Customers",
    "Similar_Item" => "منتجات مشابهة",
    'Connect_Us' => 'اتنصل بنا',
    'Featured' => 'متميز',
    'Top Selling' => 'الأكثر مبيعا',
    'What are you looking for?' => 'ما الذي تبحث عنه؟',
    'About us' => 'معلومات عنا',
    'Help' => 'مساعدة',
    'My account' => 'حسابي',
    'Blog' => 'مقالات',
    'Contacts' => 'اتصل بنا',
    'Keep in touch' => 'أبق على اتصال',
    'Follow Us' => 'تابعنا',
    'Quick Links' => 'روابط سريعة',
    'Categories' => "فئات",
    'Shop Now' => 'تسوق الآن',
    'New' => 'جديد',
    'Hot' => 'حار',
    'About_Us' => 'معلومات عنا',
    'Total' => 'المجموع',
    'Checkout' => 'الدفع',
    'Account' => 'حساب',
    'Sign In or Sign Up' => 'تسجيل الدخول أو الاشتراك',
    'My Profile' => 'ملفي',
    'My Orders' => 'طلباتي',
    'Wishlist' => 'قائمة الرغبات',
    'View Cart' => 'عرض عربة التسوق',
    'Shop_Now' => 'تسوق الان',
    "Top_Selling" => "الاكثر مبيعا",
    'Sign in' => 'تسجيل الدخول ',
    'Free shipping, IF order more then 80 JOD' => "شحن مجاني ، إذا طلب أكثر من 80 دينار",
    'dashboard' => 'لوحة التحكم',
    'New Arrivals' => "وصلنا حديثا",
    'best price' => "أفضل الاسعار",
    'Get the best deals from' => "احصل على أفضل الاسعار من ",
    'All rights reserved' => "جميع الحقوق محفوظة",
    'by' => "من خلال ",
    "To receive all the news" => "كي يصلك كل الأخبار",
    'Showing' => "عرض",
    'results' => "نتائج",
    'Search here.....' => 'البحث هنا .....',
    'Successfuly_Send' => "تم الأرسال بنجاج",
    'Subject' => "الموضوع",
    'Body' => "النص",
    'verifyCode' => "رمز التحقق",
    'view' => "عرض",
    'Related Products' => "منتجات ذات صلة",
    "Media" => "ميديا",
    "Silder" => "سلايدر",
    "Banner" => "بنر",
    "Employees" => "الموظفين",
    "Create Employees" => "انشاء موظف",
    "Salary" => " الراتب",
    "Commission" => "العمولة",
    "Created At" => "تاريخ الانشاء",
    "Amount" => "القيمة",
    "Commissions" => "العمولات",
    "Create Commission" => "انشاء عمولة ",
    "Employee ID" => "الموظف",
    "Tigers" => "النمر",
    "Draws" => "سحوبات الملابس",
    "Debt" => "الديون",
    "Debts" => "السحوبات",
    "Expenses" => "نفقات",
    "Types Of Expenses" => "انواع النفقات",
    "Sales" => "المبيعات",
    "Sales Employees" => "مبيعات الموظفين",
    "Create Draws" => "انشاء سحوبات ملابس",
    "Create Debt" => "انشاء سحب",
    "Create Expenses" => "انشاء نفقة",
    "Type ID" => "نوع",
    "Create Types Of Expenses" => "انشاء نوع نفقة",
    "Create Sales Employees" => "انشاء مبيع موظف",
    "Available debt" => "السحوبات المتاحة",
    "Total Sales" => "مجموع مبيعات",
    "Total Expenses" => "مجموع المصاريف",
    "Discounts" => "الخصومات",
    "Create Discounts" => "انشاء خصومات",
    "Working Hours" => "اوقات الدوام",
    "Create Working Hours" => "انشاء وقت دوام",
    "Start Time" => "وقت الدخول ",
    "End Time" => "وقت الخروج",
    "Start Date" => "تاريخ البداية",
    "Round Balance" => "رصيد مدور",
    "Work Start Time" => "وقت الدوام",
    "Work End Time" => "وقت انهاء الدوام",
    "Updated At" => "تاريخ التعديل",
    "Create Sales" => "انشاء مبيع",
    "Payment Method" => "طريقة الدفع",
    "Tiger" => "نمرة",
    "Sum Tiger" => "مجموع النمر",
    "Sum Expenses" => "مجموع السحوبات",
    "Monthy" => "المورقة الشهرية",
    "Time Hour" => "عدد ساعات العمل ",
    'This employee has registered his shift on this date' => 'هذا الموظف قد تم تسجيل دوامه بهذا التاريخ',
    'Month' => "الشهر",
    "salesEmployee" => "تم انشاء بيع بتاريخ  {0} من قبل {1} بقيمة {2}  دينار",
    "workingHours" => "تم ادخل وقت دوام {0} من الساعة {1} الى الساعة {2} بتاريخ {3} بتوقيت الاردن",
    "expenses_success" => "تم انشاء {0} ال البيت بقيمة {1} دينار ل {2} بتاريخ {3} لشهر {4} بتوقيت الاردن",
    "insert_tiger" => "أدخل النمر",
    'Charity Account' => "تصقية حساب"



];